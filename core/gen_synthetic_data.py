...
class dataGen_poissonOnly(object):
    ...
    def __init__(self, trNum, T=50, K0=2, K2=5, K3=3, N=7, N1=6, meanZ0Levels=[0], fixZ0=False, infer=True, setTruePar=True,add_trend=False):
        ...
        #     meanPost, covPost = inferTrial(self.cca_input, tr)
            # self.meanPost.append(meanPost)
            # self.covPost.append(covPost)
class dataGen_gaussianOnly(object):

    def __init__(self, trNum, T=50, K0=2, K2=5, K3=3, N=7, N1=6, meanZ0Levels=[0], fixZ0=False, infer=True, setTruePar=True,add_trend=False):
        super(dataGen_gaussianOnly,self).__init__()
        np.random.seed(90)
        D = 0
        ## Errors in gradient approximation have an average positive bias for each time point (due to the
        ## derivative of the exponential being monotonic). The larger the T the more the error is accumulating so
        ...
        for tr in range(trNum):
            ...
            # EDIT STARTS HERE
            ###################################################################
            R2 = np.random.uniform(0.1,1, size=W02.shape[0])
            covx2 = np.diag(np.tile(R2,T))
            mux2 = (np.einsum('ij,tj->ti', W02, z) + np.einsum('ij,tj->ti', W12, z2) + d2).flatten()
            x2 = np.random.normal(loc=mux2,scale=np.tile(R2,T)).reshape(T,W02.shape[0])#np.random.multivariate_normal(mean=mux2,cov=covx2,size=1).reshape(T,W02.shape[0]).T
            
            
            R3 = np.random.uniform(0.1,1, size=W03.shape[0])
            covx3 = np.diag(np.tile(R3,T))
            mux3 = (np.einsum('ij,tj->ti', W03, z) +np.einsum('ij,tj->ti', W13, z3) + d3).flatten()
            x3 = np.random.normal(loc=mux3,scale=np.tile(R3,T)).reshape(T,W03.shape[0])#np.random.multivariate_normal(mean=mux3,cov=covx3,size=1)
        ...
        trueObsPar = [{'W0': W02, 'W1': W12, 'd': d2, 'R':R2},
                      {'W0': W03, 'W1': W13, 'd': d3, 'R':R3}]
...
class dataGen_multiPoisson(object):

    def __init__(self, trNum, T=50, K0=2, K1=3, K2=5, K3=3, N1=7, N2=6, N3=4, meanZ0Levels=[0], fixZ0=False, infer=True,
                 setTruePar=True, add_trend=False):
        super(dataGen_multiPoisson, self).__init__()
        np.random.seed(90)
        ## Errors in gradient approximation have an average positive bias for each time point (due to the
        ## derivative of the exponential being monotonic). The larger the T the more the error is accumulating so
        # eventually the precision in the approx derivative will be lost.
        ...
        tau1 = np.random.uniform(0.2, 1, K1)
        K_big1 = makeK_big(K1, tau1, None, binSize, epsNoise=epsNoise, T=T, computeInv=False)[1]
        ...
        W01 = np.random.normal(size=(N1, K0)) / fract
        W11 = np.random.normal(size=(N1, K1)) / fract

        W12 = 1 * np.random.normal(size=(N2, K2)) / fract
        W02 = np.random.normal(size=(N2, K0)) / fract

        W03 = np.random.normal(size=(N3, K0)) / fract
        W13 = 1 * np.random.normal(size=(N3, K3)) / fract

        # R = np.random.uniform(0.1, 3, size=D)
        d1 = 1 * np.random.uniform(-1, 0.2, size=(N1))
        d2 = 1 * np.random.uniform(-1, 0.2, size=(N2))
        d3 = 1 * np.random.uniform(-1, 0.2, size=(N3))
        # A = np.random.normal(size=(R.shape[0],) * 2)
        # A = np.dot(A, A.T)
        # _, U = np.linalg.eig(A)
        #
        ...
        for tr in range(trNum):
            ...
            self.blokTr[tr] = blkId
            if (not fixZ0) or tr == 0:
                z = np.random.multivariate_normal(mean=np.ones(K0 * T) * meanZ0Levels[blkId], cov=K_big0,
                                                  size=1).reshape(K0, T).T
            if add_trend:
                z = z + yy.reshape(z.shape[0], 1)

            z1 = np.random.multivariate_normal(mean=np.zeros(K1 * T), cov=K_big1, size=1).reshape(K1, T).T
            z2 = np.random.multivariate_normal(mean=np.zeros(K2 * T), cov=K_big2, size=1).reshape(K2, T).T
            z3 = np.random.multivariate_normal(mean=np.zeros(K3 * T), cov=K_big3, size=1).reshape(K3, T).T


            x1 = np.random.poisson(lam=np.exp(np.einsum('ij,tj->ti', W01, z) +
                                              np.einsum('ij,tj->ti', W11, z1) + d1))
            ...
            # for k in range(D):
            #     preproc.covariates['var%d' % k].append(x1[:, k])
            r1 = np.exp(np.einsum('ij,tj->ti', W01, z) +
                        np.einsum('ij,tj->ti', W11, z1) + d1)
            ...
            preproc.data.append({'Y': np.hstack([x1, x2, x3])})
            preproc.data[-1]['R'] = np.hstack([r1, r2, r3])

            ground_truth_latent.append(np.hstack((z, z1, z2, z3)))

        ## infer trials

        trueStimPar = {}

        trueObsPar = [{'W0': W01, 'W1': W11, 'd': d1},
                      {'W0': W02, 'W1': W12, 'd': d2},
                      {'W0': W03, 'W1': W13, 'd': d3}]

        truePriorPar = [{'tau': tau0},{'tau': tau1}, {'tau': tau2}, {'tau': tau3}]

        # create the data structure
        self.cca_input = P_GPCCA(preproc, list(preproc.covariates.keys()), ['A', 'B','C'],
                                 np.array(['A'] * N1 + ['B']*N2 + ['C'] * N3),
                                 np.ones(preproc.ydim, dtype=bool), transposeY=False)
        self.cca_input.ground_truth_latent = ground_truth_latent
        self.cca_input.initializeParam([K0, K1, K2, K3])
...
if __name__ == '__main__':
    ...
    data = dataGen_multiPoisson(50, T=50, K0=2, K1=1, K2=1, K3=1, N1=30, N2=30, N3=30, meanZ0Levels=[0], fixZ0=False, infer=False,
                                setTruePar=True, add_trend=False)

    #data = dataGen_gaussianOnly(50,T=100, K0=2, K2=5, K3=3,N=30,N1=40,fixZ0=False,infer=False)
    # i_latent = 2
    # tr = 0
    ...
    np.savez('/Users/edoardo/Work/Code/FF_dimReduction/P-GPCCA_analyze/three_areas/tame_gp_graphical_model/3areas_tame.npz',data=data.cca_input)